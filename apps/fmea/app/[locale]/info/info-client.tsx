...
export default function FmeaInfoPage({ content }: { content: InfoPage }) {
  const locale = useLocale()
  const t = useTranslations('fmeaInfo')
  const landingUrl = `${process.env.NEXT_PUBLIC_LANDING_PAGE_URL || ''}/${locale}`

  const capabilities = [
    { icon: FileCheck, key: 'd1' },
    { icon: ShieldCheck, key: 'd2' },
    { icon: BarChart3, key: 'd3' },
    { icon: Download, key: 'd4' },
    { icon: TableProperties, key: 'd5' },
    { icon: Zap, key: 'd6' },
  ...
  return (
    <main className="min-h-dvh bg-neutral-50 text-neutral-900">
      ...
      <section className="mx-auto max-w-[860px] px-6 pb-12 pt-16 text-center">
        ...
        <h1 className="mb-5 text-3xl font-bold leading-tight tracking-tight text-neutral-900 sm:text-5xl">
      ...
      <section className="mx-auto mb-20 max-w-[960px] px-6">
        ...
        <h2 className="mb-4 text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl">
          {content.disciplinesTitle || content.section1Title}
        </h2>
        <div
          className="mb-10 max-w-none text-base leading-relaxed text-neutral-500 sm:text-lg prose prose-neutral"
          dangerouslySetInnerHTML={{ __html: content.disciplinesSub || content.section1Description || '' }}
        />
          ...
          {capabilities.map((c) => (
            ...
            >
              ...
              <h4 className="mb-2 text-[0.95rem] font-bold text-neutral-900">
                {(content as any)[`${c.key}Title`] || t(`${c.key}Title`)}
              </h4>
              <p className="text-[0.8rem] leading-relaxed text-neutral-500">
                {(content as any)[`${c.key}Desc`] || t(`${c.key}Desc`)}
              </p>
            </div>
          ))}
        </div>
      ...
      <section className="mb-20 border-y border-neutral-100 bg-white px-6 py-16">
        <div className="mx-auto max-w-[960px]">
          ...
          <h2 className="mb-10 text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl">
            {content.howTitle || content.section2Title}
          </h2>
          <div
            className="mb-10 max-w-none text-base leading-relaxed text-neutral-500 sm:text-lg prose prose-neutral"
            dangerouslySetInnerHTML={{ __html: content.howLabel ? "" : (content.section2Description || '') }}
          />
            ...
            {steps.map((s) => (
              <div key={s.key} className="flex flex-col items-start gap-5">
                ...
                <h3 className="text-xl font-bold text-neutral-900">
                  {(content as any)[`${s.key}Title`] || t(`${s.key}Title`)}
                </h3>
                <p className="text-[0.95rem] leading-relaxed text-neutral-500">
                  {(content as any)[`${s.key}Desc`] || t(`${s.key}Desc`)}
                </p>
              </div>
            ))}
          </div>
            ...
            >
              <CardContent className="flex flex-1 flex-col p-8">
                ...
                <div className="mb-3 text-[0.7rem] font-bold uppercase tracking-widest text-neutral-400">
                  {(content as any)[`${plan.key}Credits`] || t(`${plan.key}Credits`)}
                </div>
                <div className={`mb-2 font-bold text-neutral-900 ${plan.key === 'plan3' ? 'text-3xl sm:text-4xl' : 'text-4xl sm:text-5xl'}`}>
                  {(content as any)[`${plan.key}Price`] || t(`${plan.key}Price`)}{' '}
                  <span className="text-xl font-normal text-neutral-300">€</span>
                </div>
                <div className="mb-8 text-sm font-medium text-neutral-400">
                  {(content as any)[`${plan.key}Per`] || t(`${plan.key}Per`)}
                </div>
          ...
          <div className="mx-auto flex flex-col items-center gap-16 lg:flex-row lg:items-center">
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 lg:justify-start">
                <Button
                  render={<a href="#sample" className="flex items-center justify-center gap-2" />}
                  size="lg"
                ...
                <Button render={<a href={`${landingUrl}#pricing`} className="flex items-center justify-center gap-2" />} variant="outline" size="lg" className="rounded-full border-white/20 bg-white/5 text-white hover:bg-white/10 text-sm font-semibold h-12 w-full sm:w-56">
                  <Clock className="h-5 w-5" />
            ...
            <div className="relative w-full max-w-[280px] flex-shrink-0 animate-float lg:max-w-[320px]">
              <div className="rounded-3xl bg-white p-8 shadow-2xl shadow-black/40">
                <div className="mb-6 rounded-2xl bg-blue-600 px-5 py-4">
                  ...
                  <div className="text-lg font-bold text-white">{t('docHeaderSub')}</div>
                </div>
                {[
                  { label: 'SOD', title: t('docD1'), width: 'w-4/5' },
                  { label: 'AP', title: t('docD2'), width: 'w-full' },
                  { label: 'RULE', title: t('docD4'), width: 'w-3/4' },
                  { label: 'PDF/XLSX', title: t('docD5D8'), width: 'w-5/6' },
                ].map((item, idx) => (
      ...
      <section className="mx-auto mb-20 max-w-[960px] px-6">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          {faqs.map((faq, i) => (
            <Card
              key={faq.key}
              className={`border-neutral-200 bg-white shadow-sm overflow-hidden transition-all hover:shadow-md ${
                i === faqs.length - 1 && faqs.length % 2 !== 0 ? 'md:col-span-2' : ''
              }`}
            >
              <CardContent className="p-4">
                <h3 className="mb-3 text-[1rem] font-bold text-neutral-900">
                  {(content as any)[`${faq.key}Q`] || t(`${faq.key}Q`)}
                </h3>
                <p className="text-[0.9rem] leading-relaxed text-neutral-500">
                  {(content as any)[`${faq.key}A`] || t(`${faq.key}A`)}
                </p>
              </CardContent>
            </Card>
          ))}
      ...
      <section className="mx-auto max-w-[720px] px-6 pb-24 pt-10 text-center">
        <div className="rounded-[2.5rem] bg-white px-8 py-16 border border-neutral-200 shadow-sm">
          <h2 className="mb-5 text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button render={<a href={`${landingUrl}#pricing`} className="flex items-center justify-center gap-2" />} size="lg" className="rounded-full bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-600/20 text-sm font-semibold h-12 w-full sm:w-52">
              <CreditCard className="h-5 w-5" />
              {content.finalCtaBuy || t('finalCtaBuy')}
            </Button>
            <Button render={<a href={`/${locale}/reviewer`} className="flex items-center justify-center gap-2" />} variant="outline" size="lg" className="rounded-full border-blue-200 bg-white text-blue-600 hover:bg-blue-50 hover:border-blue-600 text-sm font-semibold h-12 w-full sm:w-52">
              <ArrowRight className="h-5 w-5" />
...
function CookieDeclaration({ t }: { t: ReturnType<typeof useTranslations> }) {
