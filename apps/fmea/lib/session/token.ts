...
export async function readTokenPayload(token: string): Promise<TokenPayload | null> {
  try {
    const data = await unsealData<TokenPayload>(token, {
      password: getPassword(),
    })
    return data.isAuthenticated === true ? data : null
  } catch (err) {
    console.error('[auth] Token payload read failed:', err instanceof Error ? err.message : err)
    return null
  }
}
